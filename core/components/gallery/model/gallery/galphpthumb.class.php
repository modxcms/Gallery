...
class galPhpThumb extends phpThumb {
    ...
    public function generate() {
        if (!$this->GenerateThumbnail()) {
            $this->modx->log(modX::LOG_LEVEL_ERROR,'[Gallery] phpThumb was unable to generate a thumbnail for: '.$this->cache_filename);
            return false;
        }
        return true;
    }
    ...
    public function output() {
        ...
        if (!$output) {
            $this->modx->log(modx::LOG_LEVEL_ERROR,'[Gallery] Error outputting thumbnail:'."\n".$this->debugmessages[(count($this->debugmessages) - 1)]);
        }
        return $output;
    }

...
/**
 * Class GalleryAlbumsMediaSource
 *
 * Basically a copy of the file core/components/gallery/model/gallery/galleryalbumsmediasource.class.php to make the custom mediasource work in 3.x
 * New function signature for moveObject() in 3.x
 *
 * @package Gallery\Model
 */
class GalleryAlbumsMediaSource extends modMediaSource
{
    ...
    public function getObjectsInContainer($path) {
        ...
        if ($path == '/') {
            ...
            foreach ($items as $item) {
                ...
                if (is_array($size)) {
                    $imageWidth = min($size[0], 800);
                    $imageHeight = min($size[1], 600);
                }
